  ...
  return (
    ...
    >
      <nav className="mx-auto flex h-14 w-full max-w-5xl items-center justify-between px-4">
        <div className="hover:bg-accent rounded-md p-1 w-12 md:w-24 flex items-center justify-center transition-all duration-300">
          <HomeCloud className="w-full max-w-none" rangeX={2} rangeY={1} />
        </div>
        <div className="hidden items-center gap-2 md:flex">
          {/* links and auth buttons removed as requested */}
        </div>
        <Button
          size="icon"
          variant="outline"
      ...
      <MobileMenu open={open} className="flex flex-col justify-between gap-2">
        <div className="grid gap-y-2">{/* link placeholders removed */}</div>
        <div className="flex flex-col gap-2">{/* auth buttons removed */}</div>
      </MobileMenu>
    </header>
  );
}
